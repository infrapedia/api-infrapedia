  ...
  logoUploadFiles: (files, user) => new Promise((resolve) => {
    try {
      if (files === undefined) {
        ...
        Promise.all(files.map((x) => uploadFileLogo(x.path, user, allowedExtensionsImg))).then((filesArray) => {
          resolve(filesArray.filter(Boolean));
        })
          .catch(() => { resolve([]); });
      } else { uploadFileLogo(files.path, user, allowedExtensionsImg).then((fileArray) => { resolve((fileArray !== 0) ? [fileArray] : []); }).catch(() => { resolve([]); }); }
  ...
  kmzUploadFiles: (files, user) => new Promise((resolve) => {
    try {
      if (files === undefined) {
        ...
        Promise.all(files.map((x) => uploadFileKmz(x.path, user, allowedExtensionsKmz))).then((filesArray) => {
          resolve(filesArray.filter(Boolean));
        }).catch(() => { resolve([]); });
      } else { uploadFileKmz(files.path, user, allowedExtensionsKmz).then((fileArray) => { resolve((fileArray !== 0) ? [fileArray] : []); }).catch(() => { resolve([]); }); }
    } catch (err) { resolve(err); }
  }),
};
